...
const KIND = 'project'
type ProjectType = 'generic' | 'music' | 'code' | 'research'
...
export default function Projects() {
  ...
  const visible = useMemo(() => {
    let list = interests
    if (folderId !== null) list = list.filter((i) => i.folder_id === folderId)
    if (search.trim()) {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    await api.post(`/projects/${i.id}`, { type: projectType }).catch(() => {})
    setCreating(false)
  ...
  return (
    <>
      <div className="flex flex-col h-full overflow-hidden">
        <div className="px-6 pt-5 pb-3 flex items-center gap-3 border-b border-bg-3">
          <h1 className="text-lg font-semibold text-text-1 shrink-0">
            Projects{folderPath && <><span className="font-normal text-text-3 mx-1">/</span>{folderPath}</>}
          </h1>
          <div className="relative flex-1 max-w-xs">
            <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-3 pointer-events-none" />
            <input className="input w-full pl-8 py-1.5 text-sm" placeholder="Search…"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {creating && (
            <form onSubmit={create} className="card p-4 mb-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs text-text-3 mb-1 block">Type</label>
                  <select className="input w-full" value={projectType}
          ...
          {visible.length === 0 ? (
            ...
            <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
              {visible.map((i) => (
                <ItemCard key={i.id} interest={i} onDelete={deleteItem} onArchive={archiveItem}
                  onMoveToFolder={(id) => setMovingId(id)} onEdit={editItem} />
              ))}
            </div>
          )}
        </div>
      ...
      {movingId && (
        <MoveToFolderModal folders={folders} onSelect={moveToFolder} onClose={() => setMovingId(null)} />
      )}
    </>
  )
}
